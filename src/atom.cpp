...
void Atom::set_viscosity(int narg, char **arg) {
    ...
    if (!strcmp(arg[0], "4P")){
        ...
        //set temp calcualtion style
        this->temperature_switch=0;
        //set flag to indicate temp-dependant viscosity calculation
        this->viscosity_flag=1;
    }
    ...
    else if (!strcmp(arg[0], "Arrhenius")){
        if (narg != 3) error->all(FLERR,"Viscosity input error: wrong number of arguments for Arrhenius style");
        double C1, C2;
        sscanf(arg[1],"%lg",&C1);
        sscanf(arg[2],"%lg",&C2);
        this->viscosity = new ViscosityArr(C1, C2);
        printf("Arrhenius viscosity chosen\n");
        this->temperature_switch=1;
        this->viscosity_flag=1;
