...
PairSPHIsothermal::PairSPHIsothermal(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  single_enable = 0;
  first = 1;
...
PairSPHIsothermal::~PairSPHIsothermal() {
  if (allocated) {
...
void PairSPHIsothermal::compute(int eflag, int vflag) {
...
void PairSPHIsothermal::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHIsothermal::settings(int narg, char **/*arg*/) {
  if (narg != 0)
    error->all(FLERR,
        "Illegal number of arguments for pair_style sph/isothermal");
}
...
void PairSPHIsothermal::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR,
        "Incorrect args for pair_style sph/isothermal coefficients");
  if (!allocated)
    allocate();
  ...
  double P_one = force->numeric(FLERR,arg[6]);
  printf("P_one = %lf\n", P_one);
...
double PairSPHIsothermal::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/isothermal coeffs are set");
  }
